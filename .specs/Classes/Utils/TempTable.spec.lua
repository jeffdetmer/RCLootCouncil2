...
describe("#Utils #TempTable", function()
	...
	it("ConcatAndRelease should return the content and release the table", function()
		local t = TempTable:Acquire(1, 2, "3")
		local res = TempTable:ConcatAndRelease(t, ",")
		assert.equals("1,2,3", res)
		assert.has_error(function() return t[1] end) -- Is released
		t = TempTable:Acquire(1, 2, "3")
		res = TempTable:ConcatAndRelease(t, " ")
		assert.equals("1 2 3", res)
	end)
end)
